...
type ReelsHandler struct {
	service service.ReelsService
}

func NewReelsHandler(service service.ReelsService) *ReelsHandler {
	return &ReelsHandler{
		service: service,
	}
}

func (h *ReelsHandler) GetReels(c echo.Context) error {
	gameName := c.Param("game_name")
	reels, err := h.service.GetReels(gameName)
	if err != nil {
...
func (h *ReelsHandler) SetReels(c echo.Context) error {
	gameName := c.Param("game_name")
	...
	err := h.service.SetReels(gameName, value)
	if err != nil {
